...
export const SearchVideoDetail = () => {
  ...
  const searchVideoItem = searchVideosArray?.find(
    (item) => item.id.videoId === searchVideoMatch?.params.searchId
  );
      ...
      <VideoWrap>
        <VideoPlayBox ref={videoRef}>
          <iframe
            width="100%"
            height="100%"
            src={`https://www.youtube.com/embed/${searchVideoMatch?.params.searchId}`}
          />
        </VideoPlayBox>
        <VideoPlayInfo>
          {searchVideoItem ? (
            <>
              <VideoTitle>{searchVideoItem?.snippet.title}</VideoTitle>
              <VideoDescription>
                {searchVideoItem?.snippet.description}
              </VideoDescription>
              <ExpressionItems>
                <ExpressionItem>
                  <Watched>
